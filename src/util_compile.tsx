...
export async function exec_watch(conf: CompileConfig): Promise<CompileResult> {
    ...
    return new Promise<CompileResult>((resolve, reject) => {
        ...
        exec(
            `/bin/ls ${conf.scssPath} &> /dev/null && echo "Exists" || echo "Does not exist"`,
            async (error, stdout) => {
        ...
        const watch_process = spawn(compile_cmd,{ shell: "/bin/zsh", env: { ...process.env, PATH: "/opt/homebrew/bin" }, detached: true });
    ...
    return new Promise<CompileResult>((resolve, reject) => {
        ...
        exec(
            ...
            async (error, stdout) => {
                ...
                if(killed_any){resolve({success:true, message:"Killed all Process"});}
                else{reject({success:false, message:"No Process Found"});}
