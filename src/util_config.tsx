...
export async function exec_compile(conf: CompileConfig){
    const compile_scssPath: string = conf.scssPath.toLowerCase().endsWith('.scss') ? (conf.scssPath) : (conf.scssPath + "/style.scss");
    const compile_cssPath: string = conf.cssPath.toLowerCase().endsWith('.css') ? (conf.cssPath) : (conf.cssPath + "/style.css");
    const compile_option_outputStyle: string = `--style=${conf.outputStyle}`;
    const compile_option_sourceMap: string = conf.sourceMap == "auto" ? "" : (conf.sourceMap == "none" ? "--no-source-map" : (conf.sourceMap == "inline" ? "--embed-source-map" : (conf.sourceMap == "file" ? "" : "")));
    const compile_options = compile_option_outputStyle + " " + compile_option_sourceMap;
    const compile_cmd: string = `/opt/homebrew/bin/sass ${compile_options} "${compile_scssPath}" "${compile_cssPath}"`;
